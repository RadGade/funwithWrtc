...
function redraw() {
      ...
      if (torrentData[data.infoHash].complete) {
        if (torrentData[data.infoHash].img) {
          ctx.drawImage(
            ...
            data.pos.x - data.width / 4,
            data.pos.y - data.height / 4,
            data.width / 2,
            data.height / 2
          );
  ...
  // draw usernames
  peers
    .concat({ color: color, username: username })
...
function onDown(e) {
  ...
  var x =
    e.clientX ||
    (e.changedTouches && e.changedTouches[0] && e.changedTouches[0].pageX) ||
    0;
  var y =
    e.clientY ||
    (e.changedTouches && e.changedTouches[0] && e.changedTouches[0].pageY) ||
    0;
  var p1 = { x: x, y: y };
  var p2 = {
    x: x + 0.001,
...
tracker.on("peer", function(peer) {
  ...
  function onConnect() {
    ...
    peer.on("end", onClose);
    peer.send(
      JSON.stringify({ username: username, color: color, state: state })
    );
...
function toImage(buf, cb) {
  var blob = Buffer.isBuffer(buf) ? new window.Blob([buf]) : buf;
